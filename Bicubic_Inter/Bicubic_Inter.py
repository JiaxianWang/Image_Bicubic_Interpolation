...
for j in range(1, 2 * W, 2):
    for c in range(C):
        row_F = np.matrix(
            [[img_new[int(598), int(con_sub(j, 3)), c]], [img_new[int(598), int(con_sub(j, 1)), c]],
             [img_new[int(598), int(con_add(j, 1)), c]],
             [img_new[int(598), int(con_add(j, 3)), c]]])
        coef_R = np.matrix([-1 / 16, 9 / 16, 9 / 16, -1 / 16])

        img_new[598, j, c] = np.dot(coef_R, row_F)
...
for j in range(1, 2 * W, 2):
    for c in range(C):
        matF = np.matrix(
            [[img_new[int(con_sub(597, 3)), int(con_sub(j, 3)), c],
              img_new[int(con_sub(597, 3)), int(con_sub(j, 1)), c],
              img_new[int(con_sub(597, 3)), int(add_599(j, 1)), c],
              img_new[int(con_sub(597, 3)), int(add_599(j, 3)), c]],
             [img_new[int(con_sub(597, 1)), int(con_sub(j, 3)), c],
              img_new[int(con_sub(597, 1)), int(con_sub(j, 1)), c],
              img_new[int(con_sub(597, 1)), int(add_599(j, 1)), c],
              img_new[int(con_sub(597, 1)), int(add_599(j, 3)), c]],
             [img_new[int(add_599(597, 1)), int(con_sub(j, 3)), c],
              img_new[int(add_599(597, 1)), int(con_sub(j, 1)), c],
              img_new[int(add_599(597, 1)), int(add_599(j, 1)), c],
              img_new[int(add_599(597, 1)), int(add_599(j, 3)), c]],
             [img_new[int(add_599(597, 3)), int(con_sub(j, 3)), c],
              img_new[int(add_599(597, 3)), int(con_sub(j, 1)), c],
              img_new[int(add_599(597, 3)), int(add_599(j, 1)), c],
              img_new[int(add_599(597, 3)), int(add_599(j, 3)), c]]])
        coef_h = np.matrix([-1 / 16, 9 / 16, 9 / 16, -1 / 16])
        coef_l = np.matrix([[-1 / 16], [9 / 16], [9 / 16], [-1 / 16]])
        img_new[597, j, c] = np.dot(coef_h, np.dot(matF, coef_l))

# row 597: white missing dots
for j in range(0, 2 * W - 1, 2):
    for c in range(C):
        col_F = np.matrix(
            [img_new[int(con_sub(597, 3)), int(j), c], img_new[int(597 - 1), int(j), c],
             img_new[int(597 + 1), int(j), c],
             img_new[int(add_599(597, 3)), int(j), c]])
        coef_F = np.matrix([[-1 / 16], [9 / 16], [9 / 16], [-1 / 16]])

        img_new[597, j, c] = np.dot(col_F, coef_F)
...
for i in range (4,2*H-3,2):
    for c in range (C):
        row_F = np.matrix(
            [[img_new[int(i), int(con_sub(1,3)), c]], [img_new[int(i), int(1 - 1), c]], [img_new[int(i), int(1 + 1), c]],
...
for i in range(3,2*H-4,2):
    for c in range(C):
        col_F = np.matrix(
            [img_new[int(i - 3), int(798), c], img_new[int(i - 1), int(798), c], img_new[int(i + 1), int(798), c],
             img_new[int(i + 3), int(798), c]])
        coef_F = np.matrix([[-1 / 16], [9 / 16], [9 / 16], [-1 / 16]])

        img_new[i, 798, c] = np.dot(col_F, coef_F)

# center missing white dots in column 797
for i in range (3,2*H-4,2):
    for c in range (C):
        matF = np.matrix(
            [[img_new[int(con_sub(i, 3)), int(con_sub(797, 3)), c], img_new[int(con_sub(i, 3)), int(con_sub(797, 1)), c],
              img_new[int(con_sub(i, 3)), int(con_add(797, 1)), c], img_new[int(con_sub(i, 3)), int(con_add(797, 3)), c]],
             [img_new[int(con_sub(i, 1)), int(con_sub(797, 3)), c], img_new[int(con_sub(i, 1)), int(con_sub(797, 1)), c],
              img_new[int(con_sub(i, 1)), int(con_add(797, 1)), c], img_new[int(con_sub(i, 1)), int(con_add(797, 3)), c]],
             [img_new[int(con_add(i, 1)), int(con_sub(797, 3)), c], img_new[int(con_add(i, 1)), int(con_sub(797, 1)), c],
              img_new[int(con_add(i, 1)), int(con_add(797, 1)), c], img_new[int(con_add(i, 1)), int(con_add(797, 3)), c]],
             [img_new[int(con_add(i, 3)), int(con_sub(797, 3)), c], img_new[int(con_add(i, 3)), int(con_sub(797, 1)), c],
              img_new[int(con_add(i, 3)), int(con_add(797, 1)), c], img_new[int(con_add(i, 3)), int(con_add(797, 3)), c]]])
        coef_h = np.matrix([-1 / 16, 9 / 16, 9 / 16, -1 / 16])
        coef_l = np.matrix([[-1 / 16], [9 / 16], [9 / 16], [-1 / 16]])

        img_new[i, 797, c] = np.dot(coef_h, np.dot(matF, coef_l))
# the other missing dots using row interpolation
for i in range(4,2*H-3,2):
    for c in range(C):
        row_F = np.matrix(
            [[img_new[int(i), int(797-3), c]], [img_new[int(i), int(797 - 1), c]],
             [img_new[int(i), int(797 + 1), c]],
             [img_new[int(i), int(con_add(797,3)), c]]])
        coef_R = np.matrix([-1 / 16, 9 / 16, 9 / 16, -1 / 16])

        img_new[i, 797, c] = np.dot(coef_R, row_F)

# crop the last column and last row
img_new = img_new[0:2*H-1,0:2*W-1]
print(img_new.shape)

cv2.imwrite('img_test_hr.png', img_new)
